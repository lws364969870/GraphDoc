define(['durandal/app', 'durandal/system', "durandal/composition", 'jquery', 'knockout','session','publicAjax','public','Lodop','printTools'], function (app, system, composition, $, ko) {
	var proj = {
		//初始化组件
		init : function(){
			composition.addBindingHandler("projectRecordQueryHandler", { 
				init: function (dom){
					setTimeout(function(){
...
//			    strBodyStyle +='<link rel="stylesheet" href="lib/layer/skin/default/layer.css" />';
				strBodyStyle +='<style>' + document.getElementById("printStyleRecord").innerHTML+'</style>';
            LODOP.ADD_PRINT_HTM("0mm", "0mm", "RightMargin:0mm", "BottomMargin:0mm", '<body>'+strBodyStyle + document.getElementById("printRecord").innerHTML + '</body>');
            LODOP.NewPage();
		}
	}
